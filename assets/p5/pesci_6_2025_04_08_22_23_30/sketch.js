...
let scappate = false;
let cresci = false;
let colorePesci = false;
let coloreAcqu = false;

let scrollPercent = 0;

let scrollScappateMin = 18;
let scrollScappateMax = 35;
let scrollAcquaMin = 43;
let scrollAcquaMax = 150;
let scrollCresciMin = 55;
let scrollCresciMax = 150;
let scrollColoreMin = 65;
let scrollColoreMax = 150;


    let coloresfondo = ('rgba(235, 225, 200, 1)');
    //let coloresfondo = ('rgba(235, 225, 200, 1)');

    let colorelinee = ('rgba(50, 50, 50, 0.5)');

let coloreAcqua;
let coloreRiflesso;
let posizioneRiflesso;
let velocitaRiflesso;
let spessoreRiflesso;
...
function setup() {
  ...
  // Add an initial set of PIANTE into the system
  for (let i = 0; i < quantepiante; i++) {
    let b = new Pianta(0, 0, i);
    piante.addPianta(b);
    
    
  coloreAcqua = color('rgba(1,83,133,0.5)'); // Blu acqua
  coloreRiflesso = color(255, 100); // Bianco-azzurro traslucido
  posizioneRiflesso = -1000; // Inizia fuori dallo schermo a sinistra
  velocitaRiflesso = 25;
  spessoreRiflesso = 80;
  }
...
function draw() {
  ...
  if (scrollPercent > scrollScappateMin && scrollPercent < scrollScappateMax){
   ...
   scappate = false;
    ...
    if (scrollPercent > scrollColoreMin && scrollPercent < scrollColoreMax){
    colorePesci = true;
  } else {
   colorePesci = false;
  }
  
    if (scrollPercent > scrollAcquaMin && scrollPercent < scrollAcquaMax){
    coloreAcqu = true;
  } else {
   coloreAcqu = false;
  }
  ...
  push();
  rotate(-0.2);
  // Aggiorna la posizione del riflesso
  
  if (posizioneRiflesso < width + 100) {
    posizioneRiflesso += velocitaRiflesso;
  }
  // Disegna la banda del riflesso
    blendMode(ADD);
    
  fill(coloreAcqua);
  rect(posizioneRiflesso - width*2, 0, width*2, height*10);
  fill(coloreRiflesso);
  rect(posizioneRiflesso, 0, spessoreRiflesso, height*10);
  rect(posizioneRiflesso - 30, 0, spessoreRiflesso/5, height*10);
  pop();
    } else {
   posizioneRiflesso = -1000;
  }
...
// Piante class to manage the array of all the boids
class Piante {
  constructor() {
  ...
  arc(width/2, height/2, this.size, this.size, 0, arco, PIE);

  //arc(width/2, height/2, width/2, width/2, 0, arco, PIE);
...
class Pianta {
  constructor(x, y, ind) {
    ...
    this.indice = ind;
    this.count = 0;
    let massimoRandom = random(15, 20);
    this.massimo = width/massimoRandom;
    
    this.sizemax = 60 + random(15);
  }
  ...
  update() {
        if(this.size < this.massimo){
          this.size += 1;
        } else if (this.size > this.massimo){
          this.size = this.massimo;
        }
  ...
  render() {
    ...
    let rot = map(this.indice, 0, quantepiante, 0, 2*PI)
    rotate(rot);
...
class Boid {
  ...
  render() {
    ...
    let x5 = -this.size; //Number: x-coordinate of the second control point.
    let y5 = this.size; //Number: y-coordinate of the second control point.
    let x6 = 0;         //Number: x-coordinate of the second anchor point.
    let y6 = this.size*5; //Number: y-coordinate of the second anchor point.
    
    // coda ombra
    this.theta2 = this.theta + (differenza*0.9) + pinna;
    push();
    translate(this.position.x, this.position.y);
    rotate(this.theta2 + 0.1);
    fill('rgba(0, 0, 0, 0.1)');
    noStroke();
    beginShape();
    ...
    beginShape();
    vertex(0, this.size * 2);
    vertex(-this.size/2, this.size * 3.5);
    vertex(this.size, this.size * 3.7);
    endShape(CLOSE);

    noFill();
    beginShape();
    vertex(0, this.size * 2.2);
    vertex(-this.size/2, this.size * 3.7);
    vertex(this.size, this.size * 3.7);
    endShape(CLOSE);
    ...
    //bezier(x1+1, y1, x2, y2, x3, y3, x4+1, y4, x5, y5, x6, y6);
    //bezier(-x1, y1, -x2, y2, -x3, y3, -x4, y4, x5, y5, x6, y6);
    
    //strokeWeight(5);
    // Start drawing the shape OCCHIO è dificilissimo da controllare.
    beginShape();
    vertex(0, -this.size*3);
    bezierVertex(-this.size, -this.size*2.5, -this.size, -this.size, -this.size, -this.size);
    bezierVertex(-this.size, +this.size, 0, this.size*2, 0, this.size*2);
    ...
    noFill();
    beginShape();
    vertex(0, -this.size*3.5);
    bezierVertex(-this.size, -this.size*2.6, -this.size, -this.size, -this.size, -this.size);
    bezierVertex(-this.size, +this.size, 0, this.size*2.5, 0, this.size*2);
    vertex(0, +this.size*2.2);
    //bezierVertex(0, this.size*2, -this.size, this.size, 0, this.size*2);
    bezierVertex(+this.size*1.2, this.size, +this.size*1.2, -this.size, +this.size*1.2, -this.size);
    bezierVertex(+this.size*1.2, -this.size, +this.size*1.2, -this.size*2.5, 0, -this.size*3.5);
    //vertex(0, -this.size*3);
    // Stop drawing the shape.
    endShape();
